...
def _Train(model, data_batcher):
  ...
  with tf.device('/cpu:0'):
    ...
    while not sv.should_stop() and step < FLAGS.max_run_steps:
      ...
      sys.stdout.write('train-'+str(train_step)+": ")
...
def _Eval(model, data_batcher, vocab=None):
  ...
  while True:
    ...
    summary_writer.add_summary(summaries, train_step)
    sys.stdout.write('eval-'+str(train_step) + ": ")
    running_avg_loss = _RunningAvgLoss(
        running_avg_loss, loss, summary_writer, train_step)
    if step % 100 == 0:
      summary_writer.flush()
...
def main(unused_argv):
  ...
  hps = seq2seq_attention_model.HParams(
      ...
      enc_layers=4,
      enc_timesteps=400, # 400 words
      dec_timesteps=20, # 20 words
  ...
  if hps.mode == 'train':
    ...
    model = seq2seq_attention_model.Seq2SeqAttentionModel(
        decode_mdl_hps, vocab, gpu_no=FLAGS.gpu_no)
